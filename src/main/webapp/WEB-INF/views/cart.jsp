...
<c:forEach var="product" items="${productMap}">
    ...
    <table>
        ...
        <tr><td>${product.key.price}</td><td><form action="cart" method="post">
            <img class="plusminusinput" onclick="sum('a${product.key.id}', -1)" src="static/images/minus_sign.png" width="20" height="20"/><input id="a${product.key.id}" size="2" type="text" name="quantity" value="${product.value}" style="text-align:right;"/><img class="plusminusinput" onclick="sum('a${product.key.id}', 1)" src="static/images/plus_sign.png" width="20" height="20"/><span width="6"></span>
